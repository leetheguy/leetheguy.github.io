...
describe("Board", function() {
  describe("buildMap", function() {
    return it("creates a 23x23 array as a map", function() {
      expect(app.board.map.length).toEqual(23);
      expect(app.board.map[15].length).toEqual(23);
      return expect(app.board.map[20].length).toEqual(23);
    });
  });
  ...
  describe("buildRoomArray", function() {
    ...
    it("assigns 5..9 rooms as room", function() {
      var entries;
      entries = _.chain(app.board.rooms).flatten().filter(function(room) {
        return room.name === "room";
      }).value().length;
      expect(entries).toBeLessThan(10);
      return expect(entries).toBeGreaterThan(4);
    });
    return it("assigns 14..18 rooms as hall", function() {
      var entries;
      entries = _.chain(app.board.rooms).flatten().filter(function(room) {
        return room.name === "hall";
      }).value().length;
      expect(entries).toBeLessThan(19);
      return expect(entries).toBeGreaterThan(13);
    });
  });
  ...
  describe("plotPaths", function() {
    ...
    return it("makes every non-empty room have an exit", function() {
      var exitableRooms;
  ...
  describe("mapArchitecture", function() {
    ...
    describe("halls", function() {
      ...
      it("are placed to the east", function() {
        ...
        return expect(app.board.map[12][11]).toEqual(0);
      });
      it("are placed to the south", function() {
        room.exits.south = true;
        app.board.renderHall(room);
        expect(app.board.map[11][13]).toEqual(0);
        return expect(app.board.map[11][12]).toEqual(0);
      });
  ...
  return describe("mapPointSurroundings", function() {
    ...
    it("returns null if point is on right edge of map", function() {
      return expect(app.board.mapPointSurroundings(22, 11)).toBe(null);
    });
    it("returns null if point is on bottom edge of map", function() {
      return expect(app.board.mapPointSurroundings(11, 22)).toBe(null);
    });
