...
describe("Board", function() {
  describe("buildMap", function() {
    return it("creates a 27x27 array as a map", function() {
      expect(app.board.map.length).toEqual(27);
      expect(app.board.map[15].length).toEqual(27);
      return expect(app.board.map[26].length).toEqual(27);
    });
  });
  ...
  describe("mapArchitecture", function() {
    ...
    it("places rooms accurately on the map", function() {
      ...
      return expect(app.board.map[15][15]).toEqual(1);
    });
    describe("standard room doors", function() {
      it("are placed to the north", function() {
        room.exits.north = true;
        app.board.renderRoom(room);
        return expect(app.board.map[13][11]).toEqual(2);
      });
      it("are placed to the east", function() {
        room.exits.east = true;
        app.board.renderRoom(room);
        return expect(app.board.map[15][13]).toEqual(2);
      });
      it("are placed to the south", function() {
        room.exits.south = true;
        app.board.renderRoom(room);
        return expect(app.board.map[13][15]).toEqual(2);
      });
      return it("are placed to the west", function() {
        room.exits.west = true;
        app.board.renderRoom(room);
        return expect(app.board.map[11][13]).toEqual(2);
      });
    });
    describe("halls", function() {
      it("can have a north passage", function() {
        ...
        return expect(app.board.map[13][12]).toEqual(0);
      });
      it("are placed to the east", function() {
        room.exits.east = true;
        app.board.renderHall(room);
        expect(app.board.map[15][13]).toEqual(0);
        return expect(app.board.map[14][13]).toEqual(0);
      });
      it("are placed to the south", function() {
        room.exits.south = true;
        app.board.renderHall(room);
        expect(app.board.map[13][15]).toEqual(0);
        return expect(app.board.map[13][14]).toEqual(0);
      });
      return it("are placed to the west", function() {
        room.exits.west = true;
        app.board.renderHall(room);
        expect(app.board.map[11][13]).toEqual(0);
        return expect(app.board.map[12][13]).toEqual(0);
      });
    });
    return it("renders empties", function() {
      ...
      return expect(app.board.map[13][13]).toEqual(1);
    });
  });
  return describe("mapPointSurroundings", function() {
    ...
    it("returns null if point is on right edge of map", function() {
      return expect(app.board.mapPointSurroundings(26, 13)).toBe(null);
    });
    it("returns null if point is on bottom edge of map", function() {
      return expect(app.board.mapPointSurroundings(13, 26)).toBe(null);
    });
